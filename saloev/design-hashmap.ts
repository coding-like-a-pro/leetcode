class MyHashMap {
   ...
   * if (map[index] = [key, value]) inserted[index] = true
   * */
  ...
  /**
   * Post:
   * number >=0 && number <= capacity && (map[number] = [key, value] || map[number] = undefined)
   */
  ...
  /**
   * Post:
   * if (key in map) map[pos][0] = key 
   * else if (isNeedLastPos) 0 <= pos <= capacity && map[pos] = undefined
   * else -1
   */
